...
function getUnifiedSmartMetrics(data) {
    try {
        if (!data) return null;
        try {
            if (typeof window !== 'undefined' && window.metricsWrapper && typeof window.metricsWrapper.computeAllSmartMetrics === 'function') {
                return window.metricsWrapper.computeAllSmartMetrics(data);
            }
        } catch (e) { /* fallthrough */ }
...
// --- Derived metrics accessors ---
// Safe getters for derived metrics attached at `data._analytics.derived` by the websocket processor.
function getDerivedMetric(data, key) {
    if (!data) return null;
    const a = data._analytics || data.analytics || null;
    if (!a || !a.derived) return null;
    return typeof a.derived[key] !== 'undefined' ? a.derived[key] : null;
}

function getDerivedSummary(data) {
    if (!data) return null;
    const a = data._analytics || data.analytics || {};
    return a.derived || null;
}

try { window.getDerivedMetric = getDerivedMetric; window.getDerivedSummary = getDerivedSummary; } catch (e) { }
...
document.addEventListener('AnalyticsCore:loaded', function () {
    try {
        if (typeof AnalyticsCore === 'undefined' && !(typeof window !== 'undefined' && window.metricsWrapper)) return;
        ...
        for (const coin in coinMap) {
            try {
                ...
                try {
                    if (typeof window !== 'undefined' && window.metricsWrapper && typeof window.metricsWrapper.computeAllSmartMetrics === 'function') {
                        d.analytics = window.metricsWrapper.computeAllSmartMetrics(d);
                        d._analytics = d.analytics;
                    } else if (AnalyticsCore && typeof AnalyticsCore.computeAllSmartMetrics === 'function') {
