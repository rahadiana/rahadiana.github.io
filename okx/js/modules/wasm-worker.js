            ...
            for (const url of glueCandidates) {
                try {
                    ...
                    if (head && head.ok) {
                        ...
                        const isJs = /javascript|ecmascript/.test(ct.toLowerCase());
                        if (isJs) { chosenGlue = url; break; }
                        // Not JS (likely HTML 404 page) -> record and continue
                        lastFetchErr = new Error(`HEAD ${url} -> ${head.status} content-type=${ct}`);
                        continue;
                    }
