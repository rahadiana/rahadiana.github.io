...
self.addEventListener('message', async (ev) => {
    ...
    if (msg.cmd === 'init') {
        ...
        try {
            ...
            let mod;
            try {
                mod = await import(base + '/wasm_proto.js');
                try { self.postMessage({ cmd: 'imported-glue', requestId: msg.requestId }); } catch (e) {}
            } catch (impErr) {
                self.postMessage({ cmd: 'error', error: 'dynamic import failed: ' + String(impErr), requestId: msg.requestId });
                return;
            }
            ...
            try {
                if (typeof mod.default === 'function') {
                    await mod.default(base + '/wasm_proto_bg.wasm');
                } else if (typeof mod.init === 'function') {
                    await mod.init(base + '/wasm_proto_bg.wasm');
                } else if (typeof mod.wasm_bindgen === 'function') {
                    await mod.wasm_bindgen(base + '/wasm_proto_bg.wasm');
                }
                try { self.postMessage({ cmd: 'wasm-instantiated', requestId: msg.requestId }); } catch (e) {}
            } catch (wbErr) {
