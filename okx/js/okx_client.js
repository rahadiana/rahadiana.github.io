...
OkxClient.closePositionBy = async function ({ instId, tdMode, posSide = 'net', sz } = {}) {
	...
	// ⚡ ULTRA-ROBUST POS-MODE DETECTION
	let finalPosSide = undefined;
	try {
		const acfg = await this.get('/api/v5/account/config');
		const rawMode = (acfg && acfg.data && acfg.data[0]) ? acfg.data[0].posMode : null;
		const posMode = String(rawMode || 'net').toLowerCase();

		if (posMode === 'long_short') {
			// In hedged mode, we MUST provide a side.
			// Deriving from passed side: 'long' or 'short'
			const s = String(posSide || '').toLowerCase();
			finalPosSide = (s.includes('long') || s === 'buy') ? 'long' : 'short';
		} else {
			// Default to 'net' mode behavior (do not send posSide)
			finalPosSide = undefined;
		}

		const dbg = (typeof localStorage !== 'undefined' && localStorage.getItem('okx_debug_pos') === '1');
		if (dbg) console.debug('[OKX-CLIENT] detected posMode:', posMode, 'finalPosSide:', finalPosSide);
	} catch (e) {
		console.warn('[OKX-CLIENT] Failed to detect posMode, defaulting to net-close (no posSide)', e);
		finalPosSide = undefined;
	}

	const payload = { instId, mgnMode: tdMode };
	if (finalPosSide) payload.posSide = finalPosSide;
	if (sz) payload.sz = String(sz);

	const res = await this.post('/api/v5/trade/close-position', payload);

	// ⚡ CHECK OKX INTERNAL ERROR CODES
	if (res && res.code !== '0') {
