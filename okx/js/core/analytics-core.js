...
(function() {
    ...
    /**
     * Safe z-score computation with fallback behavior.
     * Returns `fallback` when `std` is not finite or zero.
     */
    ...
    /**
     * Kyle's Lambda (price impact) using history points
     * history: array of points with { price, volBuy?, volSell?, vol? }
     */
    function computeKyleLambda(history, opts = {}) {
        ...
        // simple moving average of lambdas
        ...
        // normalize: map small lambda to 0-100 scale using heuristic
        const abs = Math.abs(smooth);
        // heuristic scaling: choose percentiles later; for now map 0..(abs*1000) to 0..100
    ...
    /**
     * VWAP and deviation bands over a history slice
     */
    ...
    // ===================== Tier-1 Normalization Helpers =====================

    function normalizeKyleLambda(input, opts = {}) {
        // input can be numeric or object from computeKyleLambda
        const val = (input && typeof input === 'object') ? (Number(input.value) || 0) : Number(input || 0);
        // Heuristic: map abs(lambda) in range 0..0.02 to 0..100
    ...
    function normalizeVWAP(vwapBands, opts = {}) {
        // prefer deviationPct (percent) where positive => above VWAP bullish
        if (!vwapBands || typeof vwapBands !== 'object') return 50;
        const dev = Number(vwapBands.deviationPct) || 0; // percent
        // map -5%..+5% to 0..100 (below => bearish)
    ...
    function normalizeCVD(cvdObj, opts = {}) {
        // cvdObj.normalized expected in -1..1 (fraction of total volume)
    ...
    function computeVPIN(history, opts = {}) {
        ...
        // VPIN conventionally 0..1, scale to 0..100
        const vp = avgAbs;
    ...
    function computeHurstExponent(history, opts = {}) {
        ...
        // linear regression slope of rs ~ ks
        const n = ks.length;
        ...
        let hurst = slope; // Hurst exponent approx = slope
    ...
    function computeDepthImbalance(snapshot, opts = {}) {
        // snapshot expected to have bidDepth and askDepth or bid_size/ask_size aliases
        ...
        const imb = (bid - ask) / total; // -1..1 (positive => more bids)
    ...
    function calculateOrderFlowImbalance(data) {
        ...
        // Combine volume and frequency imbalances; weight volume higher
    ...
    function calculateVolumeProfile(data, bins = 50, valueAreaPercent = 0.70) {
        ...
        // Robust extraction of price and volume per point
        ...
        let poc = 0; let maxVol = -1;
        for (let i = 0; i < buckets.length; i++) { if (buckets[i] > maxVol) { maxVol = buckets[i]; poc = i; } }

        // Expand around POC until we reach valueAreaPercent of total volume
        ...
        while (collected < target) {
            ...
            if (leftVol <= 0 && rightVol <= 0) break; // no more bins
            if (rightVol > leftVol) {
                right++; collected += buckets[right];
            } else {
                left--; collected += buckets[left];
            }
        }
    ...
    function detectLiquiditySweep(data) {
        ...
        // Heuristics:
        // - recent buy or sell volume >> historical avg (ratio threshold)
        // - accompanying price move in same direction (abs %)
        // - short window support (last 3 bars)
        ...
        // Thresholds
        const RATIO_SWEEP = 3.0; // recent volume >= 3x average
        const PRICE_MOVE_PCT = 0.3; // price move >= 0.3% supports sweep
        ...
        // Secondary: detect micro-sweeps where one side is dominant and total volume spike
        ...
        if (totalSpikeRatio > 5 && confidence < 30) {
            // big total spike increases confidence modestly
    ...
    function detectRegimeTransition(data) {
        ...
        // Use ATR percent and change in volDurability to detect transitions
        const atr = computeATR(history, 14);
        const avgPrice = history.length > 0 ? history.slice(-14).reduce((s, h) => s + toNum(h.last), 0) / Math.min(history.length, 14) : toNum(data.last);
        const atrPct = safeDiv(atr, avgPrice, 0) * 100; // ATR as percent

        const volDur2h = toNum(a.volDurability2h_percent) || 50;
        const volDur24h = toNum(a.volDurability24h_percent) || 50;

        // regime heuristics
        // - Low volatility & rising durability -> accumulation regime
        // - High volatility & falling durability -> distribution/transition
        // - Sustained high vol + bias -> trending regime
        ...
        const durDelta = volDur2h - volDur24h; // positive: more recent buy dominance
        ...
        if (lowVol && durDelta > 5) {
            ...
            // transitional nuance: if ATR increasing rapidly over short window
    ...
    function calculateMultiTimeframeConfluence(data, timeframes = ['120m','30m','15m','5m','1m'], weights = null) {
        // Default weights favor higher timeframes unless provided
        const defaultWeights = { '120m': 0.30, '30m': 0.25, '15m': 0.20, '5m': 0.15, '1m': 0.10 };
        const w = weights || defaultWeights;

        // Helper: compute pricePosition (0-100)
        const history = Array.isArray(data && data._history) ? data._history : (Array.isArray(data && data.history) ? data.history : []);
        let pricePos = 50;
        try {
        ...
        for (const tf of timeframes) {
            try {
                ...
                // Fallback: if no global rec, derive from pressure/pressureIndex
                if (!rec) {
        ...
        const normalized = totalWeight > 0 ? (agg / totalWeight) : 0; // -1..1
    ...
    const __G__ = (typeof window !== 'undefined') ? window : globalThis;
    __G__.AnalyticsCore = {
        // Math helpers
        safeDiv,
        toNum,
    ...
    // Export new Tier-1 helpers
    __G__.AnalyticsCore.computeKyleLambda = computeKyleLambda;
    __G__.AnalyticsCore.computeVWAPBands = computeVWAPBands;
    __G__.AnalyticsCore.computeCVD = computeCVD;
    __G__.AnalyticsCore.computeRVOL = computeRVOL;
    // Export normalization helpers
    __G__.AnalyticsCore.normalizeKyleLambda = normalizeKyleLambda;
    __G__.AnalyticsCore.normalizeVWAP = normalizeVWAP;
    __G__.AnalyticsCore.normalizeCVD = normalizeCVD;
    __G__.AnalyticsCore.normalizeRVOL = normalizeRVOL;
    __G__.AnalyticsCore.computeTier1Normalized = computeTier1Normalized;
    // Export Phase-2 helpers
    __G__.AnalyticsCore.computeVPIN = computeVPIN;
    __G__.AnalyticsCore.computeHurstExponent = computeHurstExponent;
    __G__.AnalyticsCore.computeVolumeProfilePOC = computeVolumeProfilePOC;
    __G__.AnalyticsCore.computeDepthImbalance = computeDepthImbalance;

    // Also export key functions globally for backward compatibility
    __G__.safeDiv = safeDiv;
    __G__.toNum = toNum;
    __G__.computeATR = computeATR;
    __G__.safeZScore = safeZScore;
    __G__.generateSmartSignal = generateSmartSignal;
    __G__.detectRegimeTransition = detectRegimeTransition;
