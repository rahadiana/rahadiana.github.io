...
function computeAnalytics(p) {
    ...
    // Lightweight integrations with central metrics wrapper (prefer AnalyticsCore via wrapper, fallback to derived)
    try {
        const mw = (typeof window !== 'undefined' && window.metricsWrapper) ? window.metricsWrapper : (typeof require === 'function' ? require('../modules/metrics-wrapper.cjs') : null);
        if (mw) {
            out.avgTradeSize = (typeof mw.averageTradeSize === 'function') ? mw.averageTradeSize(out.volBuy2h, out.freqBuy2h, out.volSell2h, out.freqSell2h) : null;
            const fundingRateVal = getNumeric(p, 'funding_Rate', 'funding_rate', 'funding_interestRate');
            out.fundingAPR = (typeof mw.fundingAPR === 'function') ? mw.fundingAPR(fundingRateVal) : 0;
            const nextFundingTs = getNumeric(p, 'funding_nextFundingTime', 'funding_Time', 'funding_nextFundingTime');
            out.timeToFunding = (typeof mw.timeToFunding === 'function') ? mw.timeToFunding(nextFundingTs) : { ms: 0, seconds: 0, minutes: 0, hours: 0, human: 'now' };
        }
    } catch (e) { /* non-fatal */ }
    ...
    // Advanced Tier-1 metrics (use central `metricsWrapper` when possible)
    try {
        const history = Array.isArray(p._history) ? p._history.slice(-120) : (Array.isArray(p.history) ? p.history.slice(-120) : []);
        const mw = (typeof window !== 'undefined' && window.metricsWrapper) ? window.metricsWrapper : (typeof require === 'function' ? require('../modules/metrics-wrapper') : null);

        // kyle
        if (mw && typeof mw.computeKyleLambda === 'function') {
            out.kyleLambda = mw.computeKyleLambda(history, { lookbackPeriods: 20, minSamples: 10, smoothingWindow: 5 });
        } else {
            out.kyleLambda = { valid: false };
        }

        // vwap bands
        if (mw && typeof mw.computeVWAPBands === 'function') {
            out.vwapBands = mw.computeVWAPBands(history, { lookbackPeriods: 120, stdMultiplier: 2.0, adaptiveMultiplier: true, minMultiplier: 1.5, maxMultiplier: 3.0 });
        } else {
            out.vwapBands = { valid: false };
        }

        // cvd
        if (mw && typeof mw.computeCVD === 'function') {
            out.cvd = mw.computeCVD(history, { window: 'all', normalizationMethod: 'total', smoothingPeriod: 3 });
        } else {
            out.cvd = { valid: false };
        }

        // rvol
        if (mw && typeof mw.computeRVOL === 'function') {
            try {
                out.rvol = mw.computeRVOL(history, { baselinePeriods: 14, minSamplesRequired: 10 });
            } catch (e) { out.rvol = { valid: false }; }
        } else {
        ...
        try {
            if (mw && typeof mw.computeTier1Normalized === 'function') {
                out.tier1 = mw.computeTier1Normalized({ kyleLambda: out.kyleLambda, vwapBands: out.vwapBands, cvd: out.cvd, rvol: out.rvol });
            }
        } catch (e) { /* ignore normalization errors */ }

        // Phase-2 advanced metrics
        try {
            if (mw && typeof mw.computeVPIN === 'function') {
                out.vpin = mw.computeVPIN(history, { lookbackBars: 50, minSamples: 10 });
            }
            if (mw && typeof mw.computeHurstExponent === 'function') {
                out.hurst = mw.computeHurstExponent(history, { minSamples: 50 });
            }
            if (mw && typeof mw.computeVolumeProfilePOC === 'function') {
                out.volumeProfile = mw.computeVolumeProfilePOC(history, { bins: 24 });
            }
            if (mw && typeof mw.computeDepthImbalance === 'function') {
                out.depthImbalance = mw.computeDepthImbalance((Array.isArray(p._history) ? p._history.slice(-1)[0] : p) || p);
            }
