...
(function() {
    ...
    /**
     * Format a vol-ratio value (percent) for display.
     * Accepts numeric percent (e.g. 150) or null for infinite.
     */
    function formatVolRatio(value) {
        if (value === null || value === DEFAULTS.INFINITE_RATIO) return DEFAULTS.INFINITE_DISPLAY;
        const n = Number(value);
        if (!Number.isFinite(n)) return '-';
        if (Math.abs(n) > 9999) return DEFAULTS.INFINITE_DISPLAY;
        return Math.round(n) + '%';
    }
    /**
    ...
    window.formatVolRatio = formatVolRatio;
